...
def window_reverse(windows, window_size, img_size):
    ...
    h_windows = img_size[0] // window_size
    w_windows = img_size[1] // window_size
...
def get_mask(img_size, window_size, shift_size):
    ...
    """
    img_mask = torch.zeros((1, 1, img_size[0], img_size[1]))  # 1 H W 1
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, n_channels, n_heads, img_size,
        ...
        #     # shape: [n_window, window_size**2, window_size**2]
        #     attn_mask = get_mask(self.img_size, self.window_size, self.shift_size)
        # else:
        #     attn_mask = None
    ...
    def forward(self, x):
        ...
        if self.shift_size > 0:
            # shape: [n_window, window_size**2, window_size**2]
            attn_mask = get_mask(x.size()[-2:], self.window_size, self.shift_size).to("cuda")
            # attn_mask = get_mask(x.size()[-2:], self.window_size, self.shift_size)
        else:
            attn_mask = None
        # self.register_buffer('attn_mask', attn_mask)
...
class SwinTransformerBlockStack(nn.Module):
    ...
    def forward(self, x):
        x = F.adaptive_avg_pool2d(x,
                (x.size(-2)//self.window_size*self.window_size, x.size(-1)//self.window_size*self.window_size))
...
class Up(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x1 = F.adaptive_avg_pool2d(x1, (x2.size(2), x2.size(3)))
...
class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels, img_size):
        ...
        self.up2 = nn.ConvTranspose2d(in_channels, in_channels, kernel_size=2, stride=2)
        self.conv1 = DoubleConv(2 * in_channels, in_channels)
    ...
    def forward(self, x1, x2):
        ...
        x = torch.cat([x2, x1], dim=1)
        x = self.conv1(x)
        x = F.adaptive_avg_pool2d(x, self.img_size)
        x = self.conv2(x)
        x = self.act(x)
        return x